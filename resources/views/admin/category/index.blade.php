                ...
                <tr>
                    ...
                    <td>
                        @if($obj->parent_id)
                            {{ $obj->parent->name_tm }} /
                        @endif
                        <a href="{{ route('admin.jobs.index',$obj->slug) }}" class="text-decoration-none">
                            {{ $obj->name_tm }} <i class="bi-box-arrow-up-right"></i>
                        </a>
                    </td>
                    <td>
                        @if($obj->parent_id)
                            {!! $obj->parent->name_en ?: '<span class="text-dark">' . $obj->parent->name_tm . '</span>' !!} /
                        @endif
                        {!! $obj->name_en ?: '<span class="text-success">' . $obj->name_tm . '</span>' !!}
                    </td>
                    <td>
                        <a href="{{ route('admin.jobs.index', ['category' => $obj->id, 'stock' => 1]) }}" class="text-decoration-none">
